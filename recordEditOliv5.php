...
if ($postFormId !== $formId || $postRecordId !== $recordId) {
  ...
  $debugSql[] = 'POST frm/id mismatch. GET frm=' . $formId . ' id=' . $recordId . ' POST frm=' . $postFormId . ' id=' . $postRecordId;
    ...
    $oliTimesRangeError = isset($tablesUpdates['oli_times']) ? cms_oli_times_range_error($tablesUpdates['oli_times'], $record) : null;
  ...
  <main class="cms-content">
    ...
    <?php else: ?>
      <div class="cms-card">
        ...
        <form class="pt-4" method="post" enctype="multipart/form-data" data-form-id="<?php echo cms_h((string) $formId); ?>" data-record-id="<?php echo cms_h((string) $recordId); ?>" action="<?php echo $CMS_BASE_URL; ?>/recordEditOliv5.php?frm=<?php echo cms_h((string) ($form['id'] ?? $formId)); ?>&id=<?php echo cms_h((string) $recordId); ?>">
